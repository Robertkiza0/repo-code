"""top2_selection baseline: identical retrieval (BM25 + symbol + dependency,
deduplicated, capped) and generation as the other CCEval experiments, but
selection is a fixed policy -- always the first 2 candidates in the
existing, unmodified retrieval ranking. No LLM is involved at all.

Used alongside random2_selection to determine whether LLM selection's
...
def run_one_example_top2(
    ...
    """Same retrieval as the other experiments (BM25 + symbol + dependency,
    deduplicated, capped at 12, unmodified ranking) -- selection is a fixed
    policy: the first TOP_N candidates in that ranking, no LLM call. Only
    ...
    selected_ids = [c["chunk_id"] for c in candidates[:TOP_N]]
...
def _run_single_task_top2(
    ...
    try:
        ...
        record.update(_EMPTY_TOP2_FIELDS)
        record["error"] = f"{type(e).__name__}: {e}"

    return record


def summarize_top2(task_results: List[Dict]) -> Dict:
...
def run_top2_experiment(
    ...
    """Runs the top2_selection baseline across n_tasks. Saves
    results/cceval_{n_tasks}_top2_selection.jsonl and
    results/cceval_{n_tasks}_top2_selection_summary.json; never touches any
    other experiment's own result files.
    """
    preflight_check(jsonl_path, n_tasks)

    task_results = [
        _run_single_task_top2(generator, jsonl_path, index, repos_dir, index_dir) for index in range(n_tasks)
    ]
    summary = summarize_top2(task_results)

    results_dir_path = Path(results_dir)
    save_results_jsonl(task_results, results_dir_path / f"cceval_{n_tasks}_top2_selection.jsonl")
    save_summary_json(summary, results_dir_path / f"cceval_{n_tasks}_top2_selection_summary.json")

    return {"results": task_results, "summary": summary}


def print_top2_isolation_flags() -> None:
    print("=== Data isolation verification (top2_selection) ===")
...
def print_top2_summary_table(summary: Dict) -> None:
    print("=== CCEval top2_selection summary ===")
...
def print_top2_task_table(task_results: List[Dict]) -> None:
    header = (
